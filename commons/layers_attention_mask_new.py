...
class Attention_mask(tf.keras.layers.Layer):
  ...
  def __init__(self, hidden_size, num_heads, dropout_rate):
    ...
    self.context_x_fc = tf.keras.layers.Dense(units=1, activation=None)
    self.attention_mask_my = np.load(
        '/alldata/CXxie_data/workspace/Concerto-main_upload/Concerto_main/Attune/attention_mask_zero_addcls.npz')

    dist_1 = tf.convert_to_tensor(self.attention_mask_my['dist'])
    self.forward_position_x = tf.Variable(initial_value= dist_1, name='dist')
  ...
  def call(self, query, context, attention_mask, training=False, cache=None):
    ...
    position_x = self.forward_position_x
    context_x = tf.expand_dims(self.context_x_fc(query), axis=1)  #
    log_weights = tf.sigmoid(tf.add(context_x, (tf.tile(tf.expand_dims(position_x, axis=0), [bs, 1, 1, 1]))))
    log_weights = tf.math.log(
      tf.clip_by_value(tf.divide(log_weights, tf.stop_gradient((tf.reduce_max(log_weights, axis=-1, keepdims=True)))),
                       1e-10, 1))
