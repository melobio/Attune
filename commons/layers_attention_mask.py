...
class Attention_mask(tf.keras.layers.Layer):
  ...
  """
  def __init__(self, hidden_size, num_heads, dropout_rate,attention_mask_path):
    """Constructor.
    ...
    self.attention_mask_my = np.load(attention_mask_path)#
  ...
  def call(self, query, context, attention_mask, training=False, cache=None):
    ...
    """
    #print('attention_mask shape:',attention_mask.shape) # (15, 1, 1, 28709)
    bs = attention_mask.shape[0]
    dist = self.attention_mask_my['dist']
    gene_num = dist.shape[0]
    peak_num = dist.shape[1]
    ###################################################################
    dist_1 = tf.convert_to_tensor(dist)
    dist_1 = tf.cast(dist_1, dtype='float32')
    dist_1 = tf.expand_dims(dist_1,axis=0)
    dist_1 = repeat(dist_1, '() n d -> b n d', b=bs)

    pad_1 = tf.zeros([1,1,peak_num]) #10x 10k:28708 SHARE: 19600/109475 greenleaf: 41300
    pad_1 = repeat(pad_1, '() n d -> b n d', b=bs)

    dist_2 = tf.concat([pad_1, dist_1], axis=1)  #10x 10k:5,2001,28708  SHARE:5,2001,19600
    pad_2 = tf.zeros([1, gene_num+1, 1]) #10x 10k:2001 10x 10k tf:784
    pad_2 = repeat(pad_2, '() n d -> b n d', b=bs)
    dist_3 = tf.concat([pad_2, dist_2], axis=2)  #10x 10k:5,2001,28709  SHARE:5,2001,19601
    dist_3 = tf.expand_dims(dist_3, axis=1)
    dist_tf = tf.cast(dist_3,dtype = 'float32')
    ...
    attention_weights += dist_tf
    #attention_weights = tf.nn.softmax(attention_weights, axis=3)  # 本来有
    #attention_weights = self._dropout_layer(
    #    attention_weights, training=training)  # 本来有
...
class Attention_infer(tf.keras.layers.Layer):
  """Multi-headed attention.
  ...
  def __init__(self, hidden_size, num_heads, dropout_rate,attention_mask_path):
    ...
    Args:
      ...
      dropout_rate: float scalar, dropout rate for the Dropout layers.
    """
    super(Attention_infer, self).__init__()
    self._hidden_size = hidden_size
    self._num_heads = num_heads
    self._dropout_rate = dropout_rate
    self._size_per_head = hidden_size // num_heads
  ...
  def call(self, query, context, attention_mask, training=False, cache=None):
    ...
    dist_2 = tf.concat([pad_1, dist_1], axis=1)  #10x 10k:5,2001,28708  SHARE:5,2001,19600
    pad_2 = tf.zeros([1, gene_num+1, 1]) #10x 10k/SHARE:2001 10x 10k tf:784
    pad_2 = repeat(pad_2, '() n d -> b n d', b=bs)
    dist_3 = tf.concat([pad_2, dist_2], axis=2)  #10x 10k:5,2001,28709  SHARE:5,2001,19601
    dist_3 = tf.expand_dims(dist_3, axis=1)
    dist_tf = tf.cast(dist_3,dtype = 'float32')
...
class RelativeAttention(Attention_mask):
