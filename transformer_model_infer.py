...
from commons.layers import Attention #,Attention_infer
from commons.layers_attention_mask import Attention_infer
#from commons.layers_attention_mask_new import Attention_infer
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_size, num_heads, filter_size, dropout_rate, attention_mask_path):
        ...
        self._mha_inter = Attention_infer(hidden_size, num_heads,dropout_rate,attention_mask_path)
    ...
    def call(self,
        ...
        outputs = self._mha_intra(
            query, reference, look_ahead_mask, training, cache=None)
        ...
        outputs,cache_1 = self._mha_inter(
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self,
        ...
        for i, layer in enumerate(self._stack):
            inputs,cache_1 = layer.call(inputs,
                                encoder_outputs,
                                look_ahead_mask,
                                padding_mask,
                                training,
            ...
            cache_list.append(cache_1)
        outputs = self._layernorm(inputs)
        return outputs,cache_list


class TransformerModel_infer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        Args:
          ...
          alpha: float scalar, the parameter for length normalization used in beam
            search.
        """
        super(TransformerModel_infer, self).__init__()
    ...
    def call(self, src_token_ids, tgt_token_ids):
        ...
        encoder_outputs = self._encode(src_token_ids, padding_mask,self.cls_token_peak, training=False)
        logits,crossat = self._decode(
            tgt_token_ids, encoder_outputs, padding_mask, self.cls_token_gene)
        return logits,crossat

    def _encode(self, src_token_ids, padding_mask,cls_token, training=False):
    ...
    def _decode(self, tgt_token_ids, encoder_outputs, padding_mask, cls_token):
        ...
        batch_size = tf.shape(tgt_token_ids)[0]
        src_seq_len = tf.shape(encoder_outputs)[1]
        ...
        decoder_outputs,cache_list = self._decoder(tgt_token_embeddings,
                                        encoder_outputs,
                                        look_ahead_mask,
                                        padding_mask,
                                        training=False,
                                        cache = decoding_cache
        )

        # logits = self._embedding_logits_layer(decoder_outputs, 'logits')
        return decoder_outputs, cache_list
